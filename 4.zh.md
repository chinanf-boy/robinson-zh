...
布局模块的输入是来自的样式树[第4部分](3.zh.md),它的输出又是另一棵树了*布局树*. 这使我们在迷你渲染管道中更上一层楼: 
...
罗宾逊以下列结构存储盒子的内容区域和周围区域. \[**rust 注意: ** `f32`是32位浮点类型. ]
...
我想我现在就到此为止,但不要担心: 第6部分[即将推出,并将涵盖块级布局. ](5.zh.md)一旦块布局完成,我们就可以跳到管道的下一个阶段: 绘画!
